...
<div class="space-y-5 max-w-2xl mx-auto" x-data="{ schoolType: '{{ $user->school_id ? 'existing' : 'custom' }}' }">
    ...
    <div class="flex items-center gap-2 text-xs font-bold text-slate-500">
        <a href="{{ route('profile.show', $user->username) }}" class="hover:text-orange-600 flex items-center gap-1.5 transition-colors">
        ...
        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <div class="flex items-center gap-3.5">
                    <img src="{{ $user->avatar_url }}" alt="{{ $user->name }}" class="w-12 h-12 rounded-full object-cover ring-2 ring-orange-200">
                    <input type="file" name="avatar" id="avatar" accept="image/*"
                        class="block w-full text-xs text-slate-500 file:mr-2.5 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-orange-100 file:text-orange-700 hover:file:bg-orange-200 cursor-pointer">
                </div>
            </div>

            <!-- School Section -->
            <div class="space-y-2.5 pt-3 border-t border-amber-100">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button type="button" @click="schoolType = 'existing'" 
                        :class="schoolType === 'existing' ? 'bg-orange-500 text-white shadow-sm' : 'bg-amber-50 text-slate-700 hover:bg-orange-50 hover:text-orange-600'"
                    ...
                    <button type="button" @click="schoolType = 'custom'" 
                        :class="schoolType === 'custom' ? 'bg-orange-500 text-white shadow-sm' : 'bg-amber-50 text-slate-700 hover:bg-orange-50 hover:text-orange-600'"
                        class="py-2.5 px-3 rounded-xl text-xs font-bold transition-all text-center">
                        ➕ Tambah Sekolah Baru
                    </button>
                </div>
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                @foreach($hobbies as $hobby)
                                    <label class="cursor-pointer select-none">
                                        ...
                                        <div class="px-3 py-1.5 rounded-xl text-xs font-semibold border border-amber-200 bg-amber-50/40 text-slate-700 transition-all peer-checked:bg-orange-500 peer-checked:border-orange-500 peer-checked:text-white peer-checked:shadow-xs">
                                            #{{ $hobby->name }}
                                        </div>
                                    </label>
                                @endforeach
