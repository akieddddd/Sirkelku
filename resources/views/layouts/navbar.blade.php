<!-- Top Navigation Header Component - Charcoal & Sage Green Theme -->
<header class="sk-navbar sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-slate-200 transition-all shadow-xs" x-data="{ searchOpen: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div class="hidden md:block flex-1 max-w-md mx-4">
            <form action="{{ route('communities.index') }}" method="GET" class="relative flex items-center">
        ...
        <div class="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <!-- + Sirkel Baru Button (CTA: Muted Sage) -->
            ...
            @auth
                ...
                <a href="{{ route('messages.index') }}" id="nav-messages-link" class="relative p-2 rounded-xl text-slate-600 hover:text-[#588157] hover:bg-slate-50 transition-colors flex items-center justify-center shrink-0" title="Pesan Pribadi">
                    ...
                    <span id="nav-msg-badge" class="{{ (isset($unreadMessagesCount) && $unreadMessagesCount > 0) ? '' : 'hidden' }} absolute top-1 right-1 min-w-4 h-4 px-1 bg-rose-500 text-white text-[10px] font-extrabold rounded-full flex items-center justify-center ring-2 ring-white shadow-xs">
                        {{ isset($unreadMessagesCount) && $unreadMessagesCount > 9 ? '9+' : ($unreadMessagesCount ?? 0) }}
                    </span>
                </a>
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false" class="flex items-center gap-2 p-1 rounded-xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-200">
                    ...
                    <div x-show="open" x-cloak
                        ...
                        class="absolute right-0 mt-2 w-56 rounded-2xl bg-white shadow-xl border border-slate-200 py-1.5 z-50">
                        ...
                        <div class="border-t border-slate-100 mt-1 pt-1">
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 transition-colors">
        ...
        <div x-show="searchOpen" x-cloak 
            ...
            class="absolute top-16 left-0 right-0 bg-white border-b border-slate-200 p-3 shadow-md md:hidden z-40">
            <form action="{{ route('communities.index') }}" method="GET" class="relative max-w-lg mx-auto flex items-center">
                <svg class="w-4 h-4 text-slate-400 absolute left-3 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari sirkel, hobi, atau teman mabar..." 
                    class="sk-input-search pl-9 pr-9 py-2 text-xs w-full" autofocus>
                <button type="button" @click="searchOpen = false" class="absolute right-3 text-slate-400 hover:text-slate-700 text-sm font-bold">
                    &times;
                </button>
            </form>
        </div>
