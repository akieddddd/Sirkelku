...
<div class="space-y-5">
    ...
    <div class="sk-card overflow-hidden">
        ...
        <div class="p-5 sm:p-6 relative">
                ...
                <div class="flex items-end gap-3.5">
                    ...
                    <div class="space-y-1">
                        <div class="flex items-center gap-2 flex-wrap">
                            <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900">{{ $community->name }}</h1>
                            @if($community->hobby)
                        ...
                        <p class="text-xs text-slate-500 font-medium flex items-center gap-2">
                            <span class="font-bold text-orange-600">{{ $community->members_count }} Anggota</span>
                            <span>•</span>
                ...
                <div>
                    @if($isMember)
                        <div class="flex items-center gap-2">
                            <span class="sk-badge-amber text-xs py-1 px-3">
            ...
            <div class="space-y-2.5 pt-3.5 border-t border-orange-100">
            ...
            <!-- Navigation Tabs -->
            <div class="flex items-center gap-2 pt-4 mt-4 border-t border-orange-100 overflow-x-auto">
                <a href="{{ route('communities.show', ['slug' => $community->slug, 'tab' => 'feed']) }}"
                    class="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all {{ $activeTab === 'feed' ? 'bg-orange-500 text-white shadow-xs' : 'text-slate-600 hover:bg-orange-50 hover:text-orange-600' }}">
                    Linimasa Sirkel
                </a>
                <a href="{{ route('communities.show', ['slug' => $community->slug, 'tab' => 'forum']) }}"
                    class="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all {{ $activeTab === 'forum' ? 'bg-orange-500 text-white shadow-xs' : 'text-slate-600 hover:bg-orange-50 hover:text-orange-600' }}">
                    Forum Diskusi
                </a>
                <a href="{{ route('communities.show', ['slug' => $community->slug, 'tab' => 'members']) }}"
                    class="px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all {{ $activeTab === 'members' ? 'bg-orange-500 text-white shadow-xs' : 'text-slate-600 hover:bg-orange-50 hover:text-orange-600' }}">
            ...
            @if($isMember)
                <div class="sk-card p-4">
                    <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                        ...
                        @endif
                        <textarea name="content" rows="2.5" required placeholder="Bagikan kabar atau info kegiatan khusus anggota {{ $community->name }}..."
                            class="sk-input text-xs sm:text-sm p-3"></textarea>
                        <div class="flex items-center justify-between">
                            <label class="cursor-pointer text-xs font-bold text-orange-600 hover:text-orange-700 flex items-center gap-1.5">
                    ...
                    <div class="sk-card p-4 sm:p-5 space-y-3">
                        <div class="flex items-center gap-3">
                            <a href="{{ route('profile.show', $post->user->username) }}">
                                <img src="{{ $post->user->avatar_url }}" alt="{{ $post->user->name }}" class="w-9 h-9 rounded-xl object-cover ring-2 ring-orange-200">
                            </a>
                            <div>
                                <h4 class="text-xs sm:text-sm font-bold text-slate-900">{{ $post->user->name }}</h4>
                        ...
                        @if($post->image_url)
                            <div class="rounded-xl overflow-hidden max-h-96 border border-orange-100 bg-slate-900">
                                <img src="{{ $post->image_url }}" alt="Post image" class="w-full h-auto object-cover">
                            </div>
                        @endif

                        <div class="flex items-center gap-4 pt-2.5 border-t border-orange-100 text-xs font-semibold text-slate-500">
    ...
    @elseif($activeTab === 'forum')
        <div class="space-y-4">
            <div class="flex items-center justify-between">
                <h3 class="font-extrabold text-sm text-slate-900">Utas Diskusi Sirkel</h3>
                ...
                @forelse($threads as $thread)
                    <a href="{{ route('threads.show', $thread->id) }}" class="block p-4 hover:bg-orange-50/50 transition-colors group">
                        <div class="flex items-center justify-between gap-2 mb-1">
                            <span class="sk-badge-orange">#{{ $thread->hobby ? $thread->hobby->name : 'Diskusi' }}</span>
        ...
        <div class="sk-card p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                @foreach($members as $member)
                    <div class="flex items-center justify-between p-3 rounded-xl bg-orange-50/40 border border-orange-100/80">
                        <a href="{{ route('profile.show', $member->username) }}" class="flex items-center gap-2.5 min-w-0">
                            <img src="{{ $member->avatar_url }}" alt="{{ $member->name }}" class="w-8 h-8 rounded-full object-cover ring-2 ring-orange-200">
                            <div class="min-w-0">
                                <p class="text-xs font-bold text-slate-900 truncate hover:text-orange-600 transition-colors">{{ $member->name }}</p>
                                <p class="text-[11px] text-slate-400 font-medium truncate">{{ $member->school ? $member->school->school_name : 'Pelajar' }}</p>
                            </div>
                        </a>
                        <span class="sk-badge-amber text-[10px]">
